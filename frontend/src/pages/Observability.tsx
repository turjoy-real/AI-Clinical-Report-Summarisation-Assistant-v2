// Observability — simple metrics table. The numbers themselves are Turjoy's
// (GET /api/metrics); this page only renders them.
...
export default function Observability() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">Run-level metrics from the backend stub. Educational prototype. Not for clinical use.</p>
          ...
          <tbody className="divide-y divide-slate-100">
            {(Object.keys(LABELS) as (keyof Metrics)[]).map((key) => (
              <tr key={key}>
                ...
                <td className="px-4 py-2 text-right font-mono text-slate-900">{metrics[key]}</td>
              </tr>
            ))}
          </tbody>
        </table>
